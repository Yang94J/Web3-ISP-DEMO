...
function DevApplication(props){

    const devNum = props.num;
    ...
    const handleGetList =  async () => {
      const list = await props.cbs.getList("dev");
      console.log(list);
    ...
    const handleAccept = async(id) =>{
      try{
        await props.cbs.approve("dev",id,details[id].investorAddress,true);
        setOpenS(true);
      }catch (error){
        console.log(error);
        setOpenE(true);
    ...
    const handleDeny = async(id) =>{
      try{
        ...
        await props.cbs.approve("dev",id,details[id].investorAddress,false);
        setOpenS(true);
      }catch (error){
        console.log(error);
        setOpenE(true);
    ...
    return (
      <div style={{marginTop : 20, marginBottom : 20}}>
        <Card sx={{ minWidth: 275 }}>
          ...
          <CardActions>
            <Button size="small" disabled={devNum==0} onClick={handleGetList}>View all</Button>
          </CardActions>
        </Card>
        {
          openDialog
